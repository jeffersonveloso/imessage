...
#
# The open-absinthe x86 NAC emulator (hardware-key feature) requires unicorn-engine,
# which only supports x86_64, so this image is AMD64-only.
#
# Build:
...
COPY pkg/rustpushgo/build.rs ./pkg/rustpushgo/
...
#   libssl3t64   — TLS, dynamically linked by the Rust layer (-lssl -lcrypto)
